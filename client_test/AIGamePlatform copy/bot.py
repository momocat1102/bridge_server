...
class BotChSec:
    def __init__(self):
        self.trick_table = {6:1, 7:1, 8:2, 9:2, 10:3, 11:3, 12:4, 13:4}
        ...
        self.my_hand = []
        self.remain_cards = [[i for i in range(13)],#剩餘的牌
                             [i for i in range(13)],
                             [i for i in range(13)],
                             [i for i in range(13)]]
        #開局叫牌參考變數
        ...
        self.record_change_card = -1                         # 紀錄翻開的第一張牌
    ...
    def deal(self, mode): 
        if mode == "init":
            ...
            #看看要不要叫無王
            if(sum(self.suit_point) > self.no_king_threshold['points'] and balanced):
                self.calling_suit = -1
                ...
                if suit_max <= 5:
                    self.max_trick = max(self.suit_point[self.calling_suit] // 8, 1)
                else:
    ...
    def change_first(self, revealed_card):
        ...
        #要搶(翻出王牌或是該花色前三大)
        if(revealed_card // 13 == self.trump or revealed_card_rank < 1):
            #找出我各花色最大數字跟其index
            my_bigs = [-1, -1, -1 ,-1]
    ...
    def change_second(self, revealed_card, oppo_change):
        ...
        remain_card_len = [len(self.remain_cards[i]) for i in range(4)]
        card_rank = remain_card_len[deck_suit] - self.remain_cards[deck_suit].index(deck_num)-1
        
        if(deck_suit == self.trump or card_rank < 2):
    ...
    def change_card(self, revealed_card, oppo_card):
        self.record_change_card = revealed_card
        #後手
        if oppo_card != -1:
    ...
    def dealchange(self, myget, oppo_change):
        if myget != self.record_change_card: # 我輸
            self.opponent_hand.append(self.record_change_card)
            self.opponent_hand.sort()
            self.card_state[self.record_change_card] = 1
            self.record_change_card = -1
    ...
    def play(self, oppo_play=-1):
        ...
        if oppo_play == -1:
            ...
            try:
                ...
                self.tree_table = make_tree.build_tree(self.my_hand, self.opponent_hand, self.trump, self.tree_table)
                my_play = self.tree_table[key][2]
        else:
            ...
            try:
                ...
                self.tree_table = make_tree.build_tree(self.opponent_hand, self.my_hand, self.trump, self.tree_table)
                my_play = self.tree_table[key][3][oppo_play]
...
# test = BotChSec()
# test.my_hand = [46, 32, 2, 23, 40, 42, 10, 0, 4, 15, 49, 3, 34]
# test.opponent_hand = [14, 45, 39, 17, 37, 41, 27, 21, 44, 13, 33, 16, 20]
# print(test.play())